...
const CATEGORIES = ['CACHE', 'PEAK HOURS', 'CONTEXT', 'OTHER'] as const;
...
let activeCategory: Category = 'CACHE';

function renderTipCards(listEl: HTMLElement): void {
  listEl.innerHTML = '';
  const tips = TIPS[activeCategory];
...
function renderSubnav(navEl: HTMLElement, listEl: HTMLElement): void {
  navEl.innerHTML = '';
  for (const cat of CATEGORIES) {
    const item = document.createElement('span');
    item.className = `subnav-item${cat === activeCategory ? ' active' : ''}`;
    item.textContent = cat;
    item.addEventListener('click', () => {
      activeCategory = cat;
      renderSubnav(navEl, listEl);
      renderTipCards(listEl);
    });
    navEl.appendChild(item);
  }
}

export function renderTips(container: HTMLElement): void {
  container.innerHTML = `
    <div class="page-body">
      <div class="statlist">
        <div class="subnav" id="tips-subnav" style="border-bottom: none; padding-top: 0;"></div>
        <div id="tips-list" style="max-height: 320px; overflow-y: auto;"></div>
  ...
  const navEl = container.querySelector('#tips-subnav') as HTMLElement;
  ...
  sendToExtension({ type: 'requestPageData', payload: { pageId: 'tips' } });
}
